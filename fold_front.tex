\begin{tcbposter}[
  coverage = {spread,
    boxsep = 10pt,
  },
  poster   = {showframe=false,
    columns=4,
    rows=2,
  },
  boxes    = {
    fit,
    fit basedim=10pt,
    interior empty,
    geometry nodes=true,
    before upper={\parindent10pt\noindent},
    frame code={\showPage},
  },
]

\draw [overlay,loosely dotted] ([xshift=-2em,yshift=-2em]current page.center) -- ([yshift=-2em]current page.west) ;
\draw [overlay,loosely dotted] ([xshift=-3em]$(current page.center)!0.5!(current page.east)$) node[below=-2em] {$\ointclockwise$} -- ([xshift=-3em,yshift=10em]$(current page.north)!0.5!(current page.east)$) node[above=-2em] {$\ointclockwise$} ;


\togglefalse{countedPage}
\posterbox[%
]{name=first,column=4,row=2,span=1,rowspan=1}{
  \begin{center}
    \includesvg[width=.8\textwidth]{logo}
    \par
    \vspace{3\baselineskip}
    {\Large\scshape The Rules-Light RPG$^{*}$}
    \vfill\null
    \ast\tiny It weighs 5 grams!
  \end{center}
}

\setcounter{page}{5}
\togglefalse{countedPage}
\posterbox[%
  tikz={rotate=180},
  before upper={\parindent0pt},
]{name=fifth,column=1,row=1,span=1,rowspan=1}{
  \label{skillChart}
  \begin{center}
  \rotatebox{-90}{\noindent\begin{minipage}{.5\textheight}
    \begin{tabularx}{\linewidth}{X|LLL}
      \hline
      & \textbf{Strength} & \textbf{Dexterity} & \textbf{Speed} \\\hline
      \textbf{Academics} & Orating to a massive crowd & Forgery & Courier Runs \\
      \textbf{Athletics} & Lifting heavy loads & Climbing & Sprinting \\
      \textbf{Deceit} & Intimidation & Feigning an injury & Spreading a rumour across an entire town \\
      \textbf{Stealth} & Hiding in a hay bail & Moving quietly & Escaping into a crowd \\
      \textbf{Survival} & Brace against the wind & Running on rocky terrain & Fleeing a stampede \\
      \textbf{Vigilance} & Keeping watch all night & Feeling for an exit in the dark & Searching a full forest for a particular tree \\
      \hline
    \end{tabularx}
  \end{minipage}}
  \end{center}
}

\setcounter{page}{12}

\toggletrue{countedPage}

\posterbox[tikz={rotate=180}]{name=twelfth,column=2,row=1,span=1,rowspan=1}{
  \subsubsection{\Glsfmtplural{vitalShot}}
  \glsdesc{vitalShot}.
  {\footnotesize\commonArmourChart}
  \par
  \vspace{1em}
}

\setcounter{page}{9}
\posterbox[%
  tikz={rotate=180},
  ]{name=ninth,column=3,row=1,span=1,rowspan=1}{
  \subsubsection{\Glsfmttext{combat}}
  \glsdesc{combat}.
  \par
  \vspace{1em}
  \subsubsection{\Glsfmtname{round}}
  \glsdesc{round}.
}

\setcounter{page}{8}
\posterbox[%
  before upper={\parindent0pt},
  tikz={rotate=180},
  ]%
{name=eighth,column=4,row=1,span=1,rowspan=1}{
  \subsubsection{\Glsfmtplural{bandAct}}
  \glsdesc{bandAct}.
  \par
  \vspace{1em}
  \subsubsection{\Glsfmtplural{restingaction}}
  \glsdesc{restingaction}.
  \vspace{1em}
  \par
  \subsubsection{\Glsfmttext{weight}}
  \glsdesc{weight}.
  \par
  \vspace{1em}
  \subsubsection{\Glsfmtlongpl{ep}}
  \glsdesc{ep}.
}


\togglefalse{countedPage}
\setcounter{page}{4}
\posterbox[
  before upper={\parindent0pt},
  ]{name=fourth,column=1,row=2,span=1,rowspan=1}{
  \togglefalse{countedPage}
  \begin{center}
  \rotatebox{-90}{\noindent\begin{minipage}{.5\textheight}
    \begin{tabularx}{\linewidth}{X|LLL}
      \hline
      & \textbf{Intelligence} & \textbf{Wits} & \textbf{Charisma} \\\hline
      \textbf{Academics} & Recalling facts & Resisting an enchantment spell & Storytelling \\
      \textbf{Athletics} & Finding the easiest route to climb & Identifying optimal climbing conditions & Stage acrobatics \\
      \textbf{Deceit} & Crafting a plausible lie & Making a quick excuse & Implausible lies \\
      \textbf{Stealth} & Identifying the best hiding spot & Quickly hiding & Sleuthing a banquet, uninvited \\
      \textbf{Survival} & Planning a new, hidden trail & Spotting a rare plant & Selling a map \\
      \textbf{Vigilance} & Investigating a crime scene & Spotting an illusion spell & Hearing the best gossip \\
      \hline
    \end{tabularx}
  \end{minipage}}
  \end{center}
}

\toggletrue{countedPage}

\setcounter{page}{13}
\posterbox{name=thirteenth,column=2,row=2,span=1,rowspan=1}{
  \subsubsection{\Glsfmttext{projectile}}
  \glsdesc{projectile}.
  \par
  \vspace{1em}
  \subsubsection{\Glsfmtplural{bow}}
  \glsdesc{bow}.
  \par
  \vspace{1em}
  \subsubsection{\Glsfmtplural{impromptuThrownWeapons}}
  \glsdesc{impromptuThrownWeapons}.
}

\setcounter{page}{16}
\posterbox{name=sixteenth,column=3,row=2,span=1,rowspan=1}{
  \subsubsection{\Glsfmttext{downtime}}
  \glsdesc{downtime}.
  \par
  \vspace{1em}
  \subsubsection{\Glsfmtplural{interval}}
  \glsdesc{interval}.
}

\end{tcbposter}
