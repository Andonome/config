\newglossary*{world}{Fenestra}%
\def\glsdefaulttype{world}%
...
\glsdefpostdesc{profession}{.\\\textcolor{background}{\hrulefill}}
...
\glsdefpostdesc{location}{.\textcolor{background}{\hrulefill\flourish}}
\glssetcategoryattribute{location}{glossnamefont}{textsc}

\makeglossaries

% We only want to show full descriptions (e.g. 'PC (Player Character)') in some
% books. If this file exists, the book switches to full acronyms.

\IfFileExists{rubbish/\jobname-switch-gls}{
  \setabbreviationstyle{short-sc-long-desc}
  \setabbreviationstyle[acronym]{short-sc-long-desc}
}{
  \setabbreviationstyle{short-sc-desc}
...
\longnewglossaryentry{segment}{
  ...
  Some \glsfmtplural{thread} have all of their Segments in a single \glsentrytext{region}, while others have Segments which jump between \glsfmtplural{region}.
  ...
  Once the Segments concludes, it should be marked as done with a `\glsentrytext{sgr}\hspace{-.5em}\textbackslash~', and the next Segment in the same \glsentrytext{thread} becomes ready (`\glsentrytext{sgr}')},
  }
...
\longnewglossaryentry{interval}{
  ...
  After each Interval, each \glsentrytext{pc} regenerates:
    ...
    \begin{itemize}
      ...
      \item
      The \glsentrytext{gm} rolls $1D6$ and everyone resting by a fire gains that many \glsfmtplural{fp}.
      \item
      The wind brings \glspl{mp}, and each point goes towards whoever has the most empty \glsfmtplural{mp}.
    \end{itemize}
    \manaRegenChart
    Characters who don't sleep for a day gain 3~\glsfmtplural{ep}.
    Those who don't eat lose 1~\glsfmttext{hp}},
}
...
\longnewglossaryentry{region}{
  ...
  Whenever the \glsfmtplural{pc} go from one Region to another, the \glsentrytext{gm} checks for the next available \glsentrytext{segment} in that Region (marked `\glsentrytext{sgr}')},
  }
...
\newacronym[
  description={means the number players need to roll on the dice to achieve a \emph{tie} with the task.
    ...
    \end{boxtable}
    Rolling over the Tie Number means they have their prize, rolling under means some nasty outcome is upon them, and rolling a tie means both (or neither)},
  shortplural={TNs},
  parent={action},
  prefix={a\space},
  ]{tn}{TN}{Tie Number}
...
\longnewglossaryentry{travel}{
  ...
  Each extra mile inflicts \pglsfmtlong{ep}},
}
...
\longnewglossaryentry{vigil}{
  ...
  type={mech},
  category={rules},
  description={keep a troupe safe throughout dangerous nights outside, but inflict 3~\glsfmtplural{ep}.
  The players can divide these points among their characters as they wish, rolling \pglsfmtshort{resistedaction}\glsfmttext{resistedaction} of \roll{Strength}{Vigilance}.
  On a tie, the character always hears or imagines something in the dark},
}
...
\longnewglossaryentry{foraging}{
  ...
  type={mech},
  category={rules},
  long={Roll \roll{Wits}{Survival} at \glsentrytext{tn}~%
  \ifcase\thetemperature 14\or 12\else 10\fi, and reduce travelling speed by a mile.%
  Success means one day's worth of raw food or $1D3$ days' food once cooked (\roll{Intelligence}{Cultivation}, \glsentrytext{tn}~10)},
  description={can provide verdant supplies, easily, with just one \glsentrytext{interval} of foraging.
  The troupe slows their travelling speed by a mile, then rolls \roll{Wits}{Survival} at \tn[12] (+2~Bonus during \glsentrytext{cFive}, -2~Penalty over \glsentrytext{cTwo}, and a -4~Penalty at night).

  Success means enough basic ingredients for a day's worth of raw food, or $1D3$ days' worth of cooked food.
  Preparing the meals requires an \roll{Intelligence}{Cultivation} roll at \tn[10].
  Failure means no food, and a tie means no food, but the troupe don't reduce their walking speed},
}
...
\longnewglossaryentry{navigation}{
  ...
  type={mech},
  category={rules},
  description={is automatic with a good landmark (a mountain, \glsfmttext{broch}, or weird tree all work fine).
    But using starlight, the direction of rivers, or gut instinct, demands a roll of \roll{Intelligence}{Survival} at \glsfmttext{tn}~10.
    Each Margin means the troupe can travel in the right direction for a mile as usual.
    Each Failure Margin means they're confidently incorrect for a mile},
}
...
\longnewglossaryentry{blackWalking}{
  ...
  type={mech},
  category={rules},
  description={means walking in complete darkness (usually in a cave).
  The standard travelling distance reduces to 2 miles per \glsfmttext{interval} and each extra mile inflicts twice the usual \glsfmtplural{ep}.
  Each \glsfmttext{torch} used over the journey returns a lost mile.

  With 3~\glsfmtplural{torch}, a troupe can march in the light through the \glsfmttext{interval} for the usual 5 miles, but gains 2~\glsfmtplural{ep} for each extra mile.
  With 6~\glsfmtplural{torch}, the troupe have light over the entire \glsfmttext{interval}},
}

\longnewglossaryentry{gagingCave}{
  name={Gauging Caverns},
  text={gauging the cavern},
  first={gauging a cavern},
  firstplural={gauging caverns},
  prefix={a\space},
  parent={caving},
  type={mech},
  category={rules},
...
\longnewglossaryentry{caveFire}{
  ...
  Each Failure Margin means another \glsfmttext{round} of smoke in the cavern, and another \glsfmttext{ep} for all inside},
}

%%%%% Combat

\newcommand\stackingDamageChart{
  \begin{boxtable}[|l|ccc|Y|YYY|Y|YY|Y|Y|]

    \hline
    \textbf{Damage}       & 1 & 2     & 3       & \bf 4   &  5    &   6     & 7      & \bf 8   &  9    & \dots  & \bf 12  & 13  \\
    \textbf{Damage Roll}  & 1 & 1D6-2 & 1D6-1   &     1D6 & 1D6+1 & 1D6+2   & 1D6+3  &     2D6 & 2D6+1 & \dots  &   3D6   & 3D6+1 \\
    \hline
...
\longnewglossaryentry{statblock}{
  ...
  \ifodd\value{r3}%
    \humansoldier[\npc{\T[3]\Hu}{Human Soldiers}]
  \else%
    \dwarvensoldier[\npc{\T[3]\Dw}{Dwarven Soldiers}]
  \fi%
  \iftoggle{intro}{
    \begin{boxtable}[rllll]
      \Glsfmtplural{ap}        &  = 3 & +Speed \\
      \ifnum\theheft>1
        \Glsfmtplural{ap} Cost & = & \multicolumn{3}{l}{ \Glsfmttext{weapon} \Glsfmttext{weight} } \\
      \fi
      \underline{Att}ack       & = 7 & +Dexterity & +Melee & +\Glsfmttext{weapon} \\
      \underline{Dam}age       & = D6 & +Strength & +\Glsfmttext{weapon} & \\
      Move                   & = 3 & +Athletics & & \\
    \end{boxtable}
    }{}
  ...
  Once this \glsentrytext{npc} carries \arabic{freeHP} more \glsentrytext{weight}, they receive a -1 \glsfmtplural{attribute}~Penalty},
  }
...
\newacronym[
  description={estimate of a creature's combat abilities, and the value of any \glsentrytext{monster}'s corpse},
  shortplural={CRs},
  prefix={a\space},
  name={Creature Ratings},
  nonumberlist,
  parent={statblock},
  ]{cr}{CR}{Combat Rating}
...
\newacronym[
  ...
  prefix={an\space},
  prefixfirst={a\space},
  symbol={\glsentrytext{Water}},
  user1={\expandafter\glsentrytext{Water}\llap{\checked}},
  parent={statblock},
  name={Health Points},
  ]{hp}{HP}{Health Point}
...
\longnewglossaryentry{ambush}{
  ...
  nonumberlist,
  parent={combat},
  description={can be planned with \roll{Intelligence}{Melee} against the opponent's \roll{Wits}{Melee}.
  Each Margin on the roll reduces the target enemies' \glsfmtplural{ap} by~1},
}
...
\newacronym[
  description={reduces incoming Damage, before spending \glsfmtplural{fp}.
...
\longnewglossaryentry{covering}{
  ...
  When \glsfmtplural{pc} fail a Combat roll by an amount equal to their Covering, the attack strikes an exposed spot, and their armour mitigates nothing},
}
...
\longnewglossaryentry{vitalShot}{
  ...
  category={rules},
  parent={armour},
  long={attacks which equal an enemy's \glsentrytext{tn} plus their armour's \glsentrytext{covering} ignore all \glsentrytext{dr}},
  description={are attacks which bypass armour and strike soft flesh.
  When an attack equals the \glsentrytext{tn} plus the enemy's \glsentrytext{covering}, it deals full Damage, ignoring \glsfmttext{dr}.

  For example, the \glsfmtplural{pc} enter combat, and need to hit \glsentrytext{tn}~10.
  Everyone's armour has \glsentrytext{covering}~3; so rolling `13' means a Vitals Shot with full Damage, rolling `12' means a hit (but \glsentrytext{dr} applies), rolling `9' means the \glsentrytext{pc} takes Damage, and rolling ($10 - 3 =$) `7' means the \glsentrytext{pc} takes full Damage},
  }
...
\longnewglossaryentry{projectile}{
  ...
  prefix={a\space},
  parent={combat},
  description={rolls use \roll{Dexterity}{Projectiles} versus whatever the target does to resist (e.g. \roll{Speed}{Vigilance} to run, or \roll{Dexterity}{Melee} to attack).
    For every 5~\glsfmtplural{step}' distance, the \glsentrytext{tn} increases by 1.
    On a tie, both succeed, or neither.

    Targets who don't try to avoid the projectile have \glsentrytext{tn}~7, and don't need to spend \glsfmtplural{ap} to avoid the missile},
}
...
\longnewglossaryentry{bow}{
  ...
  The Bonus to hit equals the bow's Damage, and the \glsentrytext{ap}~cost to pull it back equals its Damage plus 2},
}
...
\longnewglossaryentry{weight}{
  ...
  nonumberlist,
  parent={equipment},
  long={Once characters carry a total Weight equal to their current \glsfmtplural{hp}, they take a -1~Penalty to all \glsfmtplural{attribute}.
  Once the weight equals double their \glsfmtplural{hp} the Penalty risess to -2, and so on},
  description={estimates how easily someone can carry an item, given its mass, and how bulky it feels.
  Each time a character carries a total Weight equal to their \glsfmtplural{hp}, they suffer a -1~Penalty to all \glsfmtplural{action}.
  So a character with 5~\glsfmtplural{hp} who has items with a total Weight of 6 would have a -1~Penalty to all \glsfmtplural{action}.
  When they lose 2~\glsfmtplural{hp}, the Penalty rises to -2.

  Creatures' bodies have a Weight equal to their maximum $\glsfmtplural{hp} \times 3$ plus the Weight of all items carried},
}
...
\longnewglossaryentry{ration}{
  ...
  Each day's food has a default \glsentrytext{weight} of~1, and failure to feed inflicts 3~\glsfmtplural{ep}},
}

\longnewglossaryentry{caltrop}{
  name={Caltrops},
  text={caltrop},
  prefix={a\space},
  type={mech},
  parent={equipment},
  description={dropped on the ground have a $\frac{1}{D6}$ chance of someone stepping on them, and inflicting \dmg{-2}~Damage.
    Increasing the bags dropped increases the chances and the \glsfmttext{weight} (each bag is 1).
    Avoiding caltrops just needs a \roll{Wits}{Vigilance} roll at \glsentrytext{tn}~7, and 1~\glsfmttext{ap}},
}

\longnewglossaryentry{torch}{
  name={Torches},
  text={torch},
  plural={torches},
  prefix={a\space},
  ...
  nonumberlist,
  parent={equipment},
  description={burn for about an hour.
  A bundle of one to three torches has \glsentrytext{weight}~1.

  Torches used as \glsfmtplural{weapon} grant +1 to attack and +0~Damage},
}

\newacronym[
  description={measure how tired, sleepless, and fed-up characters feel.
  Each EP has \glsfmttext{weight}~1},
  shortplural={EP},
  prefix={an\space},
  parent={weight},
  name={Exhaustion Points},
  ...
  description={means preparing plants and field dressing the useful parts from \glsfmtplural{monster}, especially for \glsfmtplural{ingredient}.
    ...
    \begin{boxtable}[LL]
         ...
         \IfLabelExistsTF{\jobname:harvestWoodspy}{
          \Glsfmttext{woodspy} beak & Water \glsfmttext{ingredient}$=Speed$ \\
         }{}
    \end{boxtable}
    ...
    A tie means you must use the parts immediately or they spoil.

    The dressed meat has \pglsfmtshort{weight}\glsentrytext{weight} equal to the creature's \glsfmtlongpl{hp} (not including \glsfmtplural{ingredient})},
}
...
\longnewglossaryentry{talisman}{
  ...
  type={mech},
  parent={alchemy},
  description={are spells, locked in an item.
  Once the Talisman breaks the spell strikes the nearest target, although some Talismans have just enough sentience to select a target, and break themselves open in order to strike},
}
...
\longnewglossaryentry{lonelyRoad}{
  ...
  prefix={a\space},
  category={location},
  description={farms surround each town, so the roads have few \glsfmtplural{monster}.
  But farther out, the safe, surrounding, fields fall away, leaving dense forest on one or both sides.
  These liminal spaces host fewer traders and more \glsfmtplural{monster}, so people call it `the lonely road'},
}
...
\longnewglossaryentry{olm}{
  ...
  category={indexed},
  parent={deep},
  description={are blind hand-size cave-lizards.
    They crawl along walls and ceilings, gently and quietly, searching for tiny insects or the sparce vegetation in caves.
    Any light brighter than a candle causes them discomfort, and they often respond by swarming, and attacking the light (and anyone nearby) viciously},
}
...
\longnewglossaryentry{dawnDolly}{
  ...
  category={general},
  parent={village},
  description={are scarecrow-style rag-dolls which farmers often keep by their door to waggle outside first thing in the morning.
...
\longnewglossaryentry{lich}{
  ...
  description={begin as students of Death magic.
    They can slow, and eventually stop, ageing, which locks them into a permanent state of semi-death, and leaves them unable to regenerate a single \glsentrytext{mp} without draining it from someone's death},
}
...
\longnewglossaryentry{dryad}{
  ...
  Most dryads are harmless most of the time, taking more interest in local flowers than anything else.
  However, once they disconnect from society, they stop seeing people as people, and start to see them as animals.
...
\longnewglossaryentry{dragon}{
  ...
  prefix={a\space},
  parent={fiend},
  description={\ifcase\value{r0t3}%
    love flying, hunting, and glass-blowing in their spare time
  \or%
    come from a desert land over the sees, but often land in \glsfmttext{fenestra} when \glsfmtplural{storm} blow them off-course
  \or%
    each own one part of \glsfmttext{fenestra}, as they are the only real lords of the world.
    This is why people rarely see two dragons at the same time
  \else%
    initially taught the gnolls how to speak, since their jaws are similar.
    And one day the dragons will come to collect on the debt
  \fi\ldots or so they say, but they also lie about everything},
}
...
\longnewglossaryentry{griffin}{
  ...
  They have no natural cry, but can imitate almost anything they've heard},
}
...
\longnewglossaryentry{watcher}{
  name={Watcher},
  text={watcher},
  prefix={a\space},
  parent={deep},
  category={indexed},
  description={are plant-like animals who discard their slow, weak, tendrils across the \glsfmttext{deep} and release a poisonous gas},
}
...
\longnewglossaryentry{undead}{
  ...
  prefix={an\space},
  parent={lonelyRoad},
  symbol={\glsentrytext{D}},
  description={are souls trapped in a dead body by \glsentrytext{witchcraft}.
...
\providecommand\sylfsym{\ensuremath{\mathpalette\@sylfsmash{::}}}
\def\@sylfsmash#1#2{\ooalign{\hfil$#1$\Aries\hfil\cr\hfil$#1#2$\hfil\cr}}
...
\longnewglossaryentry{gDigger}{
  ...
  parent={templeOfBeasts},
  category={rank},
  description={have brought a kill to their \glsentrytext{broch}.
  In theory, they should bury the fodder who did not survive that mission, but in practice, few people leave a body},
}
...
\longnewglossaryentry{soldier}{
  ...
  parent={templeOfBeasts},
  category={rank},
  description={are the standard \glsfmtplural{guard} who journey back and forth, guarding trader caravans.
  Farmers who guard their \glsentrytext{village} can enter the \glsentrytext{templeOfBeasts} and begin at this rank immediately},
}
...
\longnewglossaryentry{cutter}{
  ...
  parent={templeOfBeasts},
  category={rank},
  description={guard caravans along the long, \glsentrytext{lonelyRoad}, or take special missions, beyond the \glsentrytext{edge}},
}
...
\longnewglossaryentry{ranger}{
  ...
  parent={templeOfBeasts},
  category={rank},
  description={travel fast, often on horseback, delivering urgent letters, or providing fast relief to \glsfmtplural{village} in immediate trouble.
  The rank is gained by hunting down a deserter from the \glsentrytext{templeOfBeasts}},
}
...
\longnewglossaryentry{abderian}{
  name={Abderian},
  category={god},
  symbol={\textcircled{\raisebox{-.3ex}{\adfflowerright}}},
  parent={ainumar},
...
\longnewglossaryentry{templeOfPoison}{
  ...
  parent={temple},
  nonumberlist,
  description={Beer, brewing and baking protect humanity from \glsentrytext{abderian}, so this temple have established a divine monopoly on the lot},
}
...
\longnewglossaryentry{seneschal}{
  ...
  sort={7},
  prefix={a\space},
  description={count every \glsfmtlong{cp} in their \glsentrytext{warden}'s estate, keep their \glsentrytext{warden} up-to-date on recent affairs, keep in contact with local \glsfmtplural{jotter}, and ensure the \glsentrytext{wheatGuild} don't cause problems},
}
...
\longnewglossaryentry{sunGuard}{
  ...
  category={rank},
  sort={2},
  description={are the upstanding soldiers who wear shiny-white tabards and protect every town from all the unwashed masses who live in it},
}
...
\longnewglossaryentry{captain}{
  ...
  category={rank},
  sort={4},
  description={lead their \glsfmtplural{sunGuard} on a dangerous mission, at least once, so they have something to talk about throughout their careers as administrators},
}

\longnewglossaryentry{sable}{
  name={Sable},
  category={god},
  symbol={\textcircled{\adforn{11}}},
  parent={ainumar},
...
\longnewglossaryentry{feller}{
  ...
  Officially they replenish hearths, but in practice provide most of the building material for towns},
}
...
\longnewglossaryentry{notary}{
  ...
  parent={templeOfFrost},
  category={rank},
  description={record transference of ownership, so people can save money, without having to transport valuable goods.
...
\longnewglossaryentry{proctor}{
  ...
  When all else fails, proctors have the ability to ban the sale of armour across a town and nearby settlements.
...
\longnewglossaryentry{yonder}{
  name={Yonder},
  category={god},
  symbol={\setcounter{track}{\value{rn3t3}}\multiply\value{track}by3\textcircled{\rotatebox{\value{track}}{\bell}}},
  parent={ainumar},
...
\longnewglossaryentry{cTwo}{
  ...
  At the half-way point -- day thirty -- a fifteen-hour eclipse plummets \glsentrytext{fenestra} into the black, frozen abyss of space.
  People say the eclipse represents \glsentrytext{sable}'s attempt to destroy \glsentrytext{fenestra}},
}
...
\longnewglossaryentry{cFour}{
  ...
  sort={4},
  parent={cycle},
  description={brings warmth, lambs, myriad insects.
  Intrepid \glsfmtplural{guard} venture past the \glsfmttext{edge} to capture wild horses, or hunt for rare plants},
}
...
\longnewglossaryentry{coldSnap}{
  ...
  During the cold, each \glsentrytext{interval} without protection inflicts 3~\glsfmtplural{ep}.
...
\longnewglossaryentry{boneCandle}{
  name={Bone Candles},
  text={bone candle},
  prefix={a\space},
  category={indexed},
  type={mech},
  parent={plant},
  description={have three life stages.
  In stage one, they form a sticky residue, attaching to creatures' hair.
  If the creature dies, they begin stage two: consuming the corpse while starting to glow faintly.
  In stage three, the glow slowly pulses, becoming stronger, then weaker, and once touched it explodes in brilliant light.

  The fungus forms a symbiotic relationship with \glsfmtplural{monster}, as many like to watch lights from a short distance.
  If anything else comes to watch the lights, then the light feed from the spilled blood.
  Or if nothing comes, then the lights will eventually feed from the starved corpse of the failed predator},
}
...
\newglossaryentry{Air}{
  ...
  nonumberlist,
  sort=W1,
  name={\rotatebox[origin=c]{180}{\ensuremath{\heartsuit}}},
  description={Air},
}
...
\def\@getinabox#1#2{\ooalign{\hfil$#1$\glsentrytext{Fate}\hfil\cr\hfil$#1#2$\hfil\cr}}
\def\deathsym{\protect{\ensuremath{\mathpalette\@getinabox{\glsentrytext{Air}}}}}

\newglossaryentry{Death}{
  type=symbols,
  nonumberlist,
  sort=W6,
  name={\protect\deathsym},
  description={Death},
}

\providecommand\mindsym{\ensuremath{\mathpalette\@getinabox\Circle}}

\newglossaryentry{Mind}{
  type=symbols,
  nonumberlist,
  sort=W7,
  name={\protect\mindsym},
  description={Mind},
}

\def\@getinapent#1#2{\ooalign{\hfil$#1$\glsentrytext{Earth}\hfil\cr\hfil$#1#2$\hfil\cr}}
\def\lifesym{\protect{\ensuremath{\mathpalette\@getinapent{\Circle}}}}

\newglossaryentry{Life}{
  type=symbols,
  nonumberlist,
  sort=W8,
  name={\protect\lifesym},
  description={Life},
}

\def\@getinatrine#1#2{\ooalign{\hfil$#1$\glsentrytext{Fire}\hfil\cr\hfil$#1#2$\hfil\cr}}
\def\forcesym{\protect{\ensuremath{\mathpalette\@getinatrine{\glsentrytext{Earth}}}}}

\newglossaryentry{Force}{
  type=symbols,
  nonumberlist,
  sort=W9,
  name={\protect\forcesym},
  description={Force},
}

\def\lightsym{\protect{\ensuremath{\mathpalette\@getinatrine{\glsentrytext{Air}}}}}

\newglossaryentry{Light}{
  type=symbols,
  nonumberlist,
  sort=WA,
  name={\protect\lightsym},
  description={Light},
}
...
\newglossaryentry{morning}{
  ...
  nonumberlist,
  name={\Leftcircle},
  description={Morning: the first \glsentrytext{interval}},
}
...
\newglossaryentry{afternoon}{
  ...
  nonumberlist,
  name={\sun},
  description={Afternoon: the second \glsentrytext{interval}},
}
...
\newglossaryentry{evening}{
  ...
  nonumberlist,
  name={\RIGHTCIRCLE},
  description={Evening: the third \glsentrytext{interval}, starts at Sunset},
}
...
\newglossaryentry{night}{
  ...
  nonumberlist,
  name={\CIRCLE},
  description={Night: the last \glsentrytext{interval}, ends at Sunrise},
}
...
\newglossaryentry{squash}{
  ...
  description={Play this \glsentrytext{thread} at the same time as the available one (i.e. not from the same \glsentrytext{thread})},
}
...
\newglossaryentry{sgn}{
  ...
  nonumberlist,
  name={\Square},
  description={\Glsentrytext{thread} is not ready yet},
}
...
\newglossaryentry{sgr}{
  ...
  nonumberlist,
  name={\CheckedBox},
  description={\Glsentrytext{thread} is ready},
}
...
\newcommand{\squash}{\gls{squash}} % squash into next segment
\newcommand{\sgr}{\glsentrytext{sgr}} % segment ready
\newcommand{\sgn}{\glsentrytext{sgn}} % segment not ready
