...
\longnewglossaryentry{blackWalking}{
  ...
  type={mech},
  category={rules},
  description={means walking through a cave in the darkness.
  Travel over the \glsfmttext{interval} requires a \roll{Dexterity}{Caving} roll at \glsfmttext{tn}~8.
  Each \glsfmttext{torch} adds a +1~Bonus, and slowing down by 1 mile adds a -1~Bonus.
  Walking an extra mile inflicts a -1~Penalty.

    So walking 7 miles in the dark means \glsfmttext{tn}~9, but using 4~\glsfmtplural{torch} would decrease the \glsfmttext{tn} to 5.

  Failure inflicts \dmg{-2}~Damage due to a twisted angle, bumping the head, or similar.
  A tie inflicts \glsfmtplural{ep} instead of Damage},
}
