...
\newglossaryentry{talisman}{
  ...
  sort={Talisman},
  description={store a spells with an activation condition.
  Some of them strike the closest appropriate target, while others have a second spell to detect a more specific target.
...
\newglossaryentry{broch}{
  ...
  text={broch},
  prefix={a\space},
  description={ a well-armed \glsentrytext{village}, stuffed and staffed by the \glsentrytext{guard}, stands between any \gls{village} lucky enough to have one, and the \glsentrytext{edge}.
  A ring of flat earth surrounds each one, which allows the \glsentrytext{guard} to loose their arrows at predators approaching civilization},
}
